(function() {
  ...
  var GLube = function() {
    ...
    function RubiksCube() {
          ...
          for (var r = 0; r < 3; r++) {
            for (var g = 0; g < 3; g++) {
              for (var b = 0; b < 3; b++) {
                var cube = this.cubes[r][g][b];
                if (
                  Math.abs(cube.coordinates[this.axisConstant] - value) <
                  MARGIN_OF_ERROR
                ) {
                  cubes.push(cube);
                }
      ...
      this.rotateLayer = function(isDouble) {
        ...
        if (!isInitializing)
          this.degrees =
            3 +
            DEGREES *
              $.easing.easeOutExpo(0, this.rotationAngle, 0, 1, fullTurn);
      ...
      this.move = function(move) {
        ...
        var inverse =
          typeof move.inverse !== "undefined" ? move.inverse : false;
    ...
    function initShaders() {
      ...
      shaderProgram.shininess = gl.getUniformLocation(
        shaderProgram,
        "shininess"
      );
    }
    ...
    function rotate(event) {
      if (rightMouseDown) {
        ...
        new_coordinates = screenToObjectCoordinates(
          event.pageX - CANVAS_X_OFFSET,
          canvas.height - event.pageY + CANVAS_Y_OFFSET
        );
        var direction = vec3.create();
        vec3.subtract(direction, new_coordinates, init_coordinates);
        vec3.normalize(direction, direction);
        rubiksCube.setRotationAxis(
          event.pageX - CANVAS_X_OFFSET,
          canvas.height - event.pageY + CANVAS_Y_OFFSET,
          direction
        );
    ...
    function startRotate(event) {
      if (event.button == LEFT_MOUSE) {
        ...
        rubiksCube.selectCube(
          event.pageX - CANVAS_X_OFFSET,
          canvas.height - event.pageY + CANVAS_Y_OFFSET
        );
        if (rubiksCube.selectedCubes.length > 0) {
          init_coordinates = screenToObjectCoordinates(
            event.pageX - CANVAS_X_OFFSET,
            canvas.height - event.pageY + CANVAS_Y_OFFSET
          );
          setTimeout(function() {
  ...
  function bind(glube, glubeElement) {
    $(glubeElement).find('canvas').each(function() {
      glube.start(this);
  ...
  function bindControls(glube, glubeElement) {
    // controls
    $(glubeElement)
      .find('.reset-cube')
      .click(function() {
